...
class ZabbixPhpRoleTest(unittest.TestCase):
    def test_ubuntu_php_versions_match_supported_releases(self) -> None:
        defaults = load_yaml("ansible/roles/components/zabbix_server/defaults/main.yml")
    ...
    def test_zabbix_packages_use_selected_php_version(self) -> None:
        defaults = (
            REPO_ROOT / "ansible/roles/components/zabbix_server/defaults/main.yml"
    ...
    def test_php_fpm_template_path_is_version_neutral(self) -> None:
        tasks = load_yaml("ansible/roles/components/zabbix_server/tasks/php-fpm.yml")
        ...
        self.assertTrue(
            (
                REPO_ROOT
                / "ansible/roles/components/zabbix_server/templates/etc/php/fpm/conf.d/99-zabbix.ini"
            ).is_file()
        )
